...
function getPriceRange(project: Project) {
  const prices = project.buildings?.flatMap((building) =>
    building.houses?.map((house) => house.price) ?? []
  );
...
function scoreCandidate(candidate: Recommendation, form: SurveyForm) {
  ...
  const projectText = `${candidate.project.name} ${candidate.project.description} ${candidate.project.location}`.toLowerCase();
  ...
  if (locationHint) {
    if (candidate.project.location.toLowerCase().includes(locationHint)) score += 4;
    if (candidate.project.description.toLowerCase().includes(locationHint)) score += 2;
  }

  if (areaType === "calm") {
    if (projectText.match(/calm|quiet|resort|retreat|green|garden|peaceful|private/)) score += 4;
    if (projectText.match(/city|downtown|bustle|busy|center/)) score -= 1;
  } else if (areaType === "busy") {
    if (projectText.match(/city|downtown|bustle|busy|center|urban|metropolitan/)) score += 4;
  ...
  if (priority === "family") {
    ...
    if (projectText.match(/luxury|exclusive|premium|five-star|high-end/)) score += 3;
  } else if (priority === "investment") {
    if (projectText.match(/investment|value|return|growth|rental|development/)) score += 3;
...
export default function Projects() {
  ...
  const overallPrices = useMemo(() => {
    const allPrices = projects.flatMap((project) =>
  ...
  useEffect(() => {
    async function fetchProjects() {
      ...
      let query = supabaseClient
        .from("projects")
        .select("*, countries(name), buildings(id, name, houses(price, floor, nb_bedrooms, nb_bathrooms))");
  ...
  function openSurvey() {
    ...
    setRecommendationText("Answer the questions to get the closest match in the selected country.");
    setForm((prev) => ({
      ...prev,
      budgetRange: "any",
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const candidates: Recommendation[] = projects.flatMap((project) =>
      project.buildings?.flatMap((building) =>
    ...
    const validCandidates = candidates.filter((candidate) => {
      return !activeCountryId || Number(activeCountryId) === candidate.project.country_id;
    });
    ...
    const best = validCandidates.reduce<Recommendation | null>((bestSoFar, candidate) => {
      if (!bestSoFar) return candidate;
      return scoreCandidate(candidate, form) > scoreCandidate(bestSoFar, form) ? candidate : bestSoFar;
    }, null);
      ...
      <div className="bg-linear-to-br from-gray-900 via-gray-800 to-orange-900 text-white py-20 px-6">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-gray-300 text-lg max-w-xl mx-auto">
            Explore our developments across the globe and get a quick personalized recommendation.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
          <div className="rounded-3xl bg-white p-5 shadow-lg">
            <h2 className="text-xl font-semibold text-slate-900">Need a recommendation?</h2>
            <p className="mt-2 text-sm text-slate-500">
              Open the survey and get the best building match based on budget, country, area type and location.
            </p>
          </div>
          <button
            type="button"
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-gray-400">{project.countries?.name}</span>
                  </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between flex-wrap gap-4 bg-orange-500 px-6 py-5 text-white">
              <div>
                <h2 className="text-2xl font-bold">Find your perfect building</h2>
                <p className="mt-2 text-sm text-orange-100">
                  Answer a few quick questions and we'll recommend the best building from our current projects.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.45fr_1fr] gap-6 p-6">
              <div>
                <div className="rounded-3xl bg-slate-50 p-6 shadow-sm mb-6">
                  <p className="text-sm text-gray-500 mb-2">Available price range</p>
                  <p className="text-2xl font-semibold text-slate-900">
                    {overallPrices.min ? `$${overallPrices.min.toLocaleString()}` : "N/A"} - {overallPrices.max ? `$${overallPrices.max.toLocaleString()}` : "N/A"}
                  </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">What price is in mind?</label>
                    <select
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Which country do you prefer?</label>
                    <select
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Do you prefer a calm or busy area?</label>
                    <select
                      value={form.areaType}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">What location do you have in mind?</label>
                    <input
                      value={form.location}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">What matters most?</label>
                    <select
                      value={form.priority}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">How many bedrooms do you prefer?</label>
                    <select
                      value={form.bedrooms}
              ...
              <div className="rounded-3xl bg-slate-950 p-6 text-white shadow-sm">
                <div className="mb-6">
                  ...
                  <p className="text-slate-300 mt-2">
                    Country: {activeCountryId ? countries.find((item) => String(item.id) === activeCountryId)?.name : "Any"}
                  </p>
                  <p className="text-slate-300 mt-1">Location: {form.location || "Any"}</p>
                </div>

                <div className="space-y-4">
                  <div className="rounded-3xl bg-slate-900 p-4">
                    <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Budget range</p>
                    <p className="mt-2 text-lg font-semibold text-slate-100">{form.budgetRange}</p>
                  </div>
                  <div className="rounded-3xl bg-slate-900 p-4">
                    <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Area type</p>
                    <p className="mt-2 text-lg font-semibold text-slate-100">{form.areaType}</p>
                  </div>
                  <div className="rounded-3xl bg-slate-900 p-4">
                    <p className="text-sm uppercase tracking-[0.2em] text-slate-400">Preference</p>
                    <p className="mt-2 text-lg font-semibold text-slate-100">{form.priority}</p>
                  </div>
                </div>

                <div className="mt-8 rounded-3xl bg-white p-5 text-slate-900 shadow-lg">
                  <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Recommendation</p>
                  <p className="mt-3 text-sm text-slate-600">{recommendationText}</p>
                  {recommendation && (
                    <div className="mt-5 rounded-3xl bg-orange-50 p-4 text-slate-900">
                      <p className="text-sm uppercase tracking-[0.2em] text-orange-500">Best building</p>
                      <h4 className="mt-2 text-xl font-bold">{recommendation.building.name}</h4>
